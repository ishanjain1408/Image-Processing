...
class CoeffNormalizedMatchingStrategy(TemplateMatchingStrategy):
    def find_matches(self, image_payload: ImagePayload, threshold: float = 0.7) -> Tuple[Image, List[Tuple[int, int]]]:
        matched_result = cv.matchTemplate(image_payload.source_image, image_payload.template_image, cv.TM_CCOEFF_NORMED)
